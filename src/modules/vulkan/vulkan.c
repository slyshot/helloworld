...
TODO:
	...
	-
		...
		Descriptors? seems like a complicated project.
		The basic problem I'm facing is multiple:
			1. I want modularity to be a priority, but each module must coordinate their descriptor/descriptorset bindings with eachother. This is mostly solved with callbacks and the vkstate
			2. Much more damning, the descriptor bindings and sets must be coordinated with the shader. It's not so bad to design things one way, and have the shader use them however they fall out, since everything is deterministic.
				However, it completely goes against the purpose of modularizing everything, anyways. 
				One possible solution on the C-side of things, although not a good one, is to allocate a single large buffer to be passed to the shaders(s), which different modules mess with.
				This sucks; it would make the shader harder to read, it's not particularly effcient, and it doesn't account for the need of VBO's.

				A better solution might be to have each module set the binding to some number in particular (instead of trying to use num_bindings). If I wanted vkstate to be involved for some reason,
				I could have it save and check for repeated use of a binding somewhere. It could give an error in that case, I suppose.
				This isn't particularly modular, but to be honest, you can only be so hot-swappable when you're playing with the same shaders. I imagine a module might be able to introduce a new shader, 
				with it's own bindings, no problem, but for multiple things to be able to pass data to the same shader, while being completely unaware of one another, and without any harm being done if any of them are removed?
				may be expecting a little too much out of this set up, at least without a lot of work around that. Heck, it may be possible to generate the shaders in realtime from the C, but I'm not doing that.

				As is, with hardcoded shaders, it seems reasonable to have hardcoded binding points, 
	-
...
void generate_instance(vk_state *vkstate) {
    ...
    char *layername = strdup("VK_LAYER_KHRONOS_validation");
    VkInstanceCreateInfo instance_info = {
		.sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,
		.pApplicationInfo = &appinfo,
		.ppEnabledExtensionNames = (const char **)req_exts,
		.enabledExtensionCount = enabled_extension_count,
	    .ppEnabledLayerNames = (const char * const *)&layername,
		.enabledLayerCount = 1,
	};
    result = vkCreateInstance(&instance_info, NULL, &vkstate->instance);
	free(layername);
    if (result != VK_SUCCESS) {
...
// { // Device create info
	...
	vkCreateDevice(vkstate->phys_dev,&device_create_info,NULL,&vkstate->log_dev);
	free(req_exts);
...
void generate_swapchain(vk_state *vkstate) {
	...
	{//get formats
		vkstate->formats_len = 0;
		vkGetPhysicalDeviceSurfaceFormatsKHR(vkstate->phys_dev, vkstate->surface, &vkstate->formats_len, NULL);
		vkstate->available_formats = malloc(sizeof(VkSurfaceFormatKHR)*vkstate->formats_len);	
		vkGetPhysicalDeviceSurfaceFormatsKHR(vkstate->phys_dev, vkstate->surface, &vkstate->formats_len, vkstate->available_formats);
		if (vkstate->formats_len == 0) {
	...
	VkSwapchainCreateInfoKHR swapchain_info = {
		...
		//TODO: Do a bit better than choosing whatever format happens to be at index 1
		.imageFormat = vkstate->available_formats[1].format,
		.imageColorSpace = vkstate->available_formats[1].colorSpace,
	};
	vkstate->screen_extent = swapchain_info.imageExtent;
	vkstate->swapchain = malloc(sizeof(VkSwapchainKHR));
	vkCreateSwapchainKHR(vkstate->log_dev, &swapchain_info,NULL, vkstate->swapchain);
...
void generate_depth_buffer_image(vk_state *vkstate) {
	...
	VkImageCreateInfo depth_buffer_image_info = {
		...
		.extent.width = vkstate->screen_extent.width,
		.extent.height = vkstate->screen_extent.height,
		.extent.depth = 1,
		.mipLevels = 1,
		.arrayLayers = 1,
	...
	for (uint32_t i = 0; i < vkstate->swapchain_imagecount; i++) {
		VkResult result = vkCreateImage(vkstate->log_dev,&depth_buffer_image_info,NULL,vkstate->depth_buffer_image+i);
		if (result != VK_SUCCESS) {
...
void generate_renderpass(vk_state *vkstate) {
	...
	{ //attachment description
		...
		attachments[0] = (VkAttachmentDescription) {
			//TODO: This just uses the same format that swapchain would use (Which is picked arbitrarily)
			//Make a function to search for an optimal format.
			.format = vkstate->available_formats[1].format,
			.samples = vkstate->max_sample_count,
	...
	vkCreateRenderPass(vkstate->log_dev,&render_pass_info,NULL,vkstate->render_pass);
}
void generate_imageviews(vk_state *vkstate) {
	...
	for (uint32_t i = 0; i < vkstate->swapchain_imagecount; i++) {
		...
		vkCreateImageView(vkstate->log_dev, &imageview_info, NULL, &(vkstate->resolve_imageviews[i]));
	}
	...
	for (uint32_t i = 0; i < vkstate->swapchain_imagecount; i++) {
		...
		vkCreateImageView(vkstate->log_dev, &imageview_info, NULL, &(vkstate->imageviews[i]));
	}
	...
	for (uint32_t i = 0; i < vkstate->swapchain_imagecount; i++) {
		VkResult result = vkCreateImageView(vkstate->log_dev, &depth_buffer_image_view_create_info, NULL, &vkstate->depth_buffer_image_view[i]);
		if (result != VK_SUCCESS) {
...
void generate_framebuffers(vk_state *vkstate) {
	...
	for (uint32_t i = 0; i < vkstate->swapchain_imagecount; i++) {
		...
		VkFramebufferCreateInfo framebuffer_infos = {
			...
			.attachmentCount = 3,
			.width = vkstate->screen_extent.width,
			.height = vkstate->screen_extent.height,
			.layers = 1,
		};
		vkCreateFramebuffer(vkstate->log_dev, &framebuffer_infos, NULL, &(vkstate->framebuffers[i]));
	}
}
void generate_command_pool(vk_state *vkstate) {
	...
	vkCreateCommandPool(vkstate->log_dev, &command_pool_create_info, NULL, &vkstate->command_pool);
}
void generate_command_buffers(vk_state *vkstate) {
	...
	vkstate->cmd_buffer_count = vkstate->swapchain_imagecount;
	vkstate->cmd_buffers = malloc(sizeof(VkCommandBuffer)*cmd_allocate_info.commandBufferCount);
	vkAllocateCommandBuffers(vkstate->log_dev, &cmd_allocate_info, vkstate->cmd_buffers);
}


VkShaderModule create_shader_module(vk_state *vkstate, char *path) {
	int size;
	uint32_t * vertex_code;
	read_file(path, &size , &vertex_code);
	...
	VkShaderModule out;
	vkCreateShaderModule(vkstate->log_dev, &vertex_shader_module_create_info, NULL, &out);
	return out;
}
...
void generate_graphics_pipeline(vk_state *vkstate) {
	...
	VkPipelineShaderStageCreateInfo vertex_pipeline_shader_stage_create_info = {
		...
		.module = create_shader_module(vkstate, VERTEX_SHADER_PATH),
		.pName = "main",
	};
	VkPipelineShaderStageCreateInfo fragment_pipeline_shader_stage_create_info = {
		.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
		.stage = VK_SHADER_STAGE_FRAGMENT_BIT,
		.module = create_shader_module(vkstate, FRAGMENT_SHADER_PATH),
		.pName = "main",
	};
	...
	*vkstate->scissor = (VkRect2D) {
		.extent = (VkExtent2D){.width = vkstate->screen_extent.width, .height = vkstate->screen_extent.height},
	...
	result = vkCreateDescriptorPool(vkstate->log_dev, &uniform_pool_info, NULL, vkstate->uniform_descriptor_pool);
	if (result != VK_SUCCESS) {
	...
	result = vkCreateDescriptorPool(vkstate->log_dev, &sampler_pool_info, NULL, vkstate->combined_image_sampler_descriptor_pool);
	if (result != VK_SUCCESS) {
		LOG_ERROR("Failed to create descriptor pool for image sampler buffer. Errno %d\n",result);
	}			
	...
	vkCreatePipelineLayout(vkstate->log_dev, &pipeline_layout_create_info, NULL, vkstate->pipeline_layout);
	...
	VkGraphicsPipelineCreateInfo graphics_pipeline_create_info = {
		...
		.pStages = (VkPipelineShaderStageCreateInfo[2]){vertex_pipeline_shader_stage_create_info, fragment_pipeline_shader_stage_create_info},
	...
	vkCreateGraphicsPipelines(vkstate->log_dev, VK_NULL_HANDLE, 1, &graphics_pipeline_create_info, NULL, &vkstate->graphics_pipeline);
}
void generate_semaphores(vk_state *vkstate) {
	...
	vkCreateSemaphore(vkstate->log_dev, &semaphore_create_info, NULL, &(vkstate->semaphores_finished[0]));
	vkCreateSemaphore(vkstate->log_dev, &semaphore_create_info, NULL, &(vkstate->semaphores_finished[1]));
	vkCreateSemaphore(vkstate->log_dev, &semaphore_create_info, NULL, &(vkstate->semaphores_finished[2]));
	vkCreateSemaphore(vkstate->log_dev, &semaphore_create_info, NULL, &(vkstate->semaphores_available[0]));
	vkCreateSemaphore(vkstate->log_dev, &semaphore_create_info, NULL, &(vkstate->semaphores_available[1]));
	vkCreateSemaphore(vkstate->log_dev, &semaphore_create_info, NULL, &(vkstate->semaphores_available[2]));
...
void generate_fences(vk_state *vkstate) {
	...
	vkCreateFence(vkstate->log_dev, &fence_create_info, NULL, &vkstate->fences[0]);
	vkCreateFence(vkstate->log_dev, &fence_create_info, NULL, &vkstate->fences[1]);
	vkCreateFence(vkstate->log_dev, &fence_create_info, NULL, &vkstate->fences[2]);
}
...
void vulkan_init(void) {
	/*
		TODO:
			...
			Might be useful to create a dependency tree with this purpose, as a structure within the code.
			Then, to generate a given thing, it'd automatically generate the dependencies. To destroy something, it'd automatically destroy the dependencies first.
			Just a thought.
	*/
...
void vulkan_update(int dt) { 
	...
	VkRenderPassBeginInfo render_pass_begin_info = {
		...
		.renderArea.extent = vkstate.screen_extent,
