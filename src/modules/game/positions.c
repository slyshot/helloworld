...
void positions_vertex_attribute_callback(void) {
	...
	for (int i = 0; i < 4; i++) {
		vkstate.vertex_input_attribute_descriptions[num_attr_descs+i] = (VkVertexInputAttributeDescription) {
		    .binding = num_binding_descs,
		    .location = num_attr_descs+i, // Ensure these locations match the layout in your shader
		    .format = VK_FORMAT_R32G32B32A32_SFLOAT, // Each column is a vec4
		    .offset = sizeof(float) * 4 * i, // Offset for each column
		};
	}
